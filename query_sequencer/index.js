...
const indentity = async (statment, head) => { head.meta.push({ processor: 'indentity', took: 0 }) }

async function processDataStoreSearch(head, search, aggregation, timePicker, options = {}) {
...
async function executeQuery(query, timePicker, options) {
  ...
  while (iterator < totalLength) {
    ...
    if (currentItem.statement === 'search') {
      ...
      for (const query of prepareQueries(baseQuery, searchMode, head)) {
        await processDataStoreSearch(head, { ...currentItem.body, query }, nextItem && nextItem.body, timePicker, options)
      }
