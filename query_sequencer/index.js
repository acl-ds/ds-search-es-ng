    ...
    if (new_path && rvrs_new_path) {
      ...
      for (i = 0; i < new_path.length; i++) {
        ...
        if (new_path[i] !== rvrs_new_path[rvrs_new_path.length-i-1]) {
          const  rvrs_new_path_length=rvrs_new_path[rvrs_new_path.length - i - 1].length;
          const new_path_length=new_path[i].length
          if(rvrs_new_path_length===new_path_length) 
            {
                path = `${new_path[i].slice(0,new_path[i].length / 2 + 1)}${rvrs_new_path[rvrs_new_path.length - i - 1].slice(rvrs_new_path[rvrs_new_path.length - i - 1].length / 2)}`;
            }
            else {
             path=new_path_length>rvrs_new_path_length?new_path[i]:rvrs_new_path[i]
            }
