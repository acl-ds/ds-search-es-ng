...
async function storeToElasticSearch(data, esClient) {
  const index = `.search_head_${Math.random().toString(36).substr(2, 8).toLowerCase()}`
...
async function execute(command, head, options = {}) {
  ...
  const searchBody = { query: '*', size: 0, searchMode: 'default', bypassTimeFilter: true, index: tempIndex }
  const { data } = await process(searchBody, aggregationBody, { field: '' }, { esClient })
  deleteIndex(tempIndex, esClient)

  // Storing back the data
  head.data = data
  head.meta.push({ processor: 'aggregate', took: performance.now() - startTime })
}
